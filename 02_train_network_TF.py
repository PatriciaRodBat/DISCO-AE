...
from utils.model_eval_functions_TF import model_id_gen, FMN_id_gen, get_emb_size, def_model, eval_yalm, save_pred_emb_and_loss

from utils.model_eval_functions_TF import eval_network
import time
...
def train_network(cfg):
    ...
    cache_FMN = osp.join(cache_dir, FMN_id_gen(cfg) + "_model_TF")
    ...
    save_dir_name = f"trained_model_TF_{model_id}"
    model_save_path = osp.join(base_path, "data", save_dir_name, "ep" + "_{}.pth")
    save_dir_full = osp.join(base_path, "data", save_dir_name)
    os.makedirs(save_dir_full, exist_ok=True)

    #model_save_path = osp.join(save_dir_full, f"ep_{{}}_{cfg["model"]["module"]}.pth")

    log_path = os.path.join(base_path, "data", save_dir_name, "log.txt")
    log_file = open(log_path, "w")

    #log_dir = f"logs/fit/{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}_{model_id}_{cfg["model"]["module"]}"
...
if __name__ == "__main__":
    ...
    if osp.isfile(filename):
        ...
        eval_network(trained_network, cfg, calculate_errors=True, calculate_errors_p2s=True,
