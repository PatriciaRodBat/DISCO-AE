...
class FMN_Dataset(Dataset):
    ...
    def __init__(
        ...
        self.last_eig  = last_eig
        self.eig_first = eig_first
        self.eig_last  = eig_last

        #If eig_last is not filled in YAML, it is computed.
        if self.last_eig > 0 and self.eig_first > 0 and self.eig_last == 0:
            self.eig_last = self.last_eig - self.eig_first
        ...
        if use_cache:
            ...
            # if 'TRUCK' in name:
            last_eig = int(getattr(self, "last_eig", self.nCCLB))
            f = int(getattr(self, "eig_first", 0))
            l = int(getattr(self, "eig_last", max(0, last_eig - f)))
            ...
            eig_tag = f"eigF{f}L{l}"


            # # referece p2p maps version
            load_cache = os.path.join(
                self.cache_dir,
                f"cache_{dataname_seed}_{self.ndim}_{self.nCCLB}_c{self.maxsize_circle}_onlytrain{fmn_only_train}_{self.k_eig}_template_{self.template_feature_size}{self.template_features}_{split}_{train_ref}_{p2p_maps.split('/')[-1].split('.')[0]}_eig{eig_tag}.pt",
            )
        ...
        for idx in tqdm(listi):
            ...
            CCLB_full = self.FM_meshes.CCLB[idx]          #shape: (ndim, nCCLB)
            if last_eig and (f > 0 or l > 0):
                parts = []
                if f > 0: parts.append(CCLB_full[:, :f])              #1st f columns
                if l > 0: parts.append(CCLB_full[:, -l:])             #last l columns
                CCLB_cols = np.hstack(parts) if len(parts) > 1 else parts[0]   #(ndim, f+l)
            else:
                #Without slicing: using every CCLB column (nCCLB)                
                CCLB_cols = CCLB_full                                  #(ndim, nCCLB)

            #CCLB_last = CCLB_full[:, -self.last_eig:]     #shape: (ndim, last_eig)
            self.Y_pinv_list += [np.linalg.pinv(CCLB_cols)]   #(f+l, ndim) or (nCCLB, ndim)
        ...
        for idx in tqdm(self.templates_ids):
            ...
            CCLB_full = self.FM_meshes.CCLB[idx]              #(ndim, nCCLB)
            if last_eig and (f > 0 or l > 0):
                parts = []
                if f > 0: parts.append(CCLB_full[:, :f])
                if l > 0: parts.append(CCLB_full[:, -l:])
                CCLB_cols = np.hstack(parts) if len(parts) > 1 else parts[0]   #(ndim, f+l)
            else:
                CCLB_cols = CCLB_full
                                                   #(ndim, nCCLB)
            
            self.template_Y_list.append(CCLB_cols)  #(ndim, f+l) or (ndim, nCCLB)
            #self.template_Y_list += [CCLB_full[:, -self.last_eig:]]  #(ndim, last_eig)
