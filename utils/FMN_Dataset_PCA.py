...
class FMN_Dataset(Dataset):
    ...
    train: boolean
    """
    def create_static_pca(self):
        #CREATE PCA STATIC PROJECTION --> creates a fixed proyection matrix (U_pca) that will be applied to the learnt features.
        print("Creating static PCA projection from template's CCLB basis...")
                
        #1. Obtain the basis CCLB of the template.
        template_idx = self.templates_ids[0]
        Y_template = self.FM_meshes.CCLB[template_idx]
                
        #2. Apply the same snippet used for the rest of the code.
        #start_index = Y_template.shape[1] - self.nCCLB
        #Y_template_sliced = Y_template[:, start_index:]

        #3. Adjust PCA on the basis vectors.
        k = self.cfg['model']['n_pca_components']
        pca = PCA(n_components=k, whiten=False, svd_solver="auto", random_state=0)
        pca.fit(Y_template)

        U_pca  = torch.tensor(pca.components_.astype(np.float32))
        mu_pca = torch.tensor(pca.mean_.astype(np.float32))

        #print("Static PCA projection created with shape:", U_pca.shape)
        print("Static PCA projection created. U_pca:", tuple(U_pca.shape), "mu_pca:", tuple(mu_pca.shape))
    ...
    def __init__(
        ...
        k_eig=128,
        FM_meshes=None,
        p2p_maps="data/FAUST_perfect_p2p.pickle",
        name="FAUST-inter-sup",
        ndim=3,
        nCCLB=45,
        maxsize_circle=None,
        trainsplit=0.8,
        ...
        if use_cache:
            ...
            load_cache = os.path.join(
                self.cache_dir,
                f"cache_{dataname_seed}_{self.ndim}_{self.nCCLB}_c{self.maxsize_circle}_onlytrain{fmn_only_train}_{self.k_eig}_template_{self.template_feature_size}{self.template_features}_{split}_{train_ref}_{p2p_maps.split('/')[-1].split('.')[0]}_pcaV2.pt",
            )
        ...
        if self.FM_meshes is None:
            if split == "train":
                ...
                self.U_pca, self.mu_pca = self.create_static_pca()

                #For the test dataset to inherit both.
                self.FM_meshes.U_pca  = self.U_pca
                self.FM_meshes.mu_pca = self.mu_pca

                print("\n--- Matrix U_pca (Shape: {}): ---".format(self.U_pca.shape))
                print(self.U_pca)
                print("-------------------------------------\n")

                #Add to object FMN to pass it to test_dataset.
                self.FM_meshes.U_pca = self.U_pca
            ...
            if hasattr(self.FM_meshes, 'U_pca'):
                self.U_pca = self.FM_meshes.U_pca
            else:
                self.U_pca = None

            if hasattr(self.FM_meshes, 'mu_pca'):
                self.mu_pca = self.FM_meshes.mu_pca
            else:
                self.mu_pca = None

            
        # # define mesh-wise variables necessary for the network
        self.verts_list = []
        self.numverts_list = []
        self.faces_list = []
        self.center_mass_list = []
        ...
        op_cache_dir = osp.join(cache_dir, "cache")
    ...
    def __getitem__(self, idx):
        ...
        shape['U_pca'] = self.U_pca
        shape["mu_pca"] = self.mu_pca
