...
from utils.FMN_Dataset_eigvct import FMN_Dataset, shape_to_device
...
from model_eigvct import LS_DF_net
...
def model_id_gen(cfg):
    ...
    try:
        module_short = osp.splitext(cfg["model"]["module"])[0]
    except Exception:
        module_short = str(cfg["model"].get("module", "unknown"))

    #Eig-selection
    last_eig = int(cfg["fmnet"].get("last_eig", cfg["fmnet"]["nCCLB"]))
    f = int(cfg["fmnet"].get("eig_first", 0))
    l = int(cfg["fmnet"].get("eig_last", max(0, last_eig - f)))
    if f < 0: f = 0
    if l < 0: l = 0
    if f + l != last_eig:
        l = max(0, last_eig - f)
    ...
    model_id = (
        "{}_{}_{}-{}_{}_fmnet_{}_last{}_{}_c{}_onlytrain_{}_diffnet_{}_{}_{}{}_train_{}_{}".format(
            module_short,
            cfg["model"]["name"],
            cfg["dataset"]["name"],
            cfg["training"]["seed"],
    ...
    if (
        ...
        model_id = "{}_{}_{}_fmnet_{}_last{}_{}_c{}_onlytrain_{}_diffnet_{}_{}_LL_{}_{}{}_train_{}_{}".format(
            module_short,
            cfg["model"]["name"],
            cfg["dataset"]["name"],
            cfg["fmnet"]["ndim"],
...
def FMN_id_gen(cfg):
    cache_FMN = "cache_{}_FMN_{}_{}".format(
        cfg["dataset"]["name"], cfg["fmnet"]["ndim"], cfg["fmnet"]["last_eig"], cfg["fmnet"]["nCCLB"]
    )
...
def get_emb_size(cfg):
    last_eig = int(cfg["fmnet"].get("last_eig", cfg["fmnet"]["nCCLB"]))
    f = int(cfg["fmnet"].get("eig_first", 0))
    l = int(cfg["fmnet"].get("eig_last", last_eig - f))
    #In case
    if f < 0: f = 0
    if l < 0: l = 0
    if f + l != last_eig:
...
def eval_network(
    ...
    train_dataset = FMN_Dataset(
        ...
        train=True,
        verbose=True,
        last_eig=cfg["fmnet"]["last_eig"],
        eig_first=cfg["fmnet"]["eig_first"],
        eig_last=cfg["fmnet"]["eig_last"]
    )
