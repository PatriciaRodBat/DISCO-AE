...
from utils.FMN_Dataset_PCA import FMN_Dataset, shape_to_device
...
def def_model(cfg, device):
    #Creates an instance of the model based on the config file.
    #Reads the name of the file (module) and class from the config.yaml to load the correct model.
    module_name = cfg['model']['module']
    class_name = cfg['model']['name']

    #Imports the module (file .py).
    module = importlib.import_module(module_name)
    
    ModelClass = getattr(module, class_name)

    #Creates the instance of the model and sends it to the device.
    ae_network = ModelClass(cfg).to(device)
...
def get_emb_size(cfg):
    #Shows the final size of embedding space after PCA projection. 
    txt2 = "Embedding Size: {:,} * {:,} = {:,}".format(
        cfg["model"]["n_pca_components"],
        cfg["diffnet"]["nfeature"],
        cfg["model"]["n_pca_components"] * cfg["diffnet"]["nfeature"],
    )
    return txt2
...
#Receives train_dataset to pass FM_meshes (which contains U_pca) to test_dataset and avoid errors.
