...
def dataloader_GALLOP(select_parts, select_tt, datadir="data", verbose=False, ndim=100):
    ...
    for part in select_parts:
        ...
        """
        # load data (cache isolated per SLURM task if provided)
        cache_root = os.environ.get("DISCO_DATALOADER_CACHE_DIR", osp.join(datadir, "cache"))
        os.makedirs(cache_root, exist_ok=True)

        # filename (avoid putting the full array in the filename)
        cache = osp.join(cache_root, f"meshes_{datapath}_n{len(select_tt)}_ndim{ndim}.pickle")
