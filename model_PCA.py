...
class LS_DF_net(nn.Module):
    ...
    def forward(self, batch):
        #1: Data extraction.
        #Data from input figure (for the encoder):
        ...
        #PCA static projection (precomputed on FMN_Dataset_PCA.py):
        U_pca = batch["shape"]["U_pca"]
        mu_pca = batch["shape"]["mu_pca"]


        #2: ENCODER (Mesh -> Matrix emb).
        ...
        #3: PCA reduction (Matrix emb -> Latent Vector).

        if U_pca.dim() == 3: #Erase the extra batch dimension
            U_pca = U_pca[0]
        if mu_pca.dim() == 2: mu_pca = mu_pca[0]

        #Move U_pca to the same device as model
        U_pca = U_pca.to(emb.device)
        mu_pca = mu_pca.to(emb.device, emb.dtype)
        
        #Projection
        emb_centered = emb - mu_pca.unsqueeze(-1)
        latent_code  = torch.matmul(U_pca, emb_centered)
        #latent_code = torch.matmul(U_pca, emb)
        

        #4: DECODER (Latent Vector -> 3D Mesh).
        #Inverse to projection
        reconstructed_emb = torch.matmul(U_pca.transpose(0, 1), latent_code) + mu_pca.unsqueeze(-1)
        #reconstructed_emb = torch.matmul(U_pca.transpose(0, 1), latent_code)

        #TEMPORARY CHECK
        difference = torch.mean((emb - reconstructed_emb)**2)
        print(f"Mean squared difference between emb and reconstructed_emb: {difference.item()}")
        #
