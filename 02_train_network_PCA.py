...
from utils.FMN_Dataset_PCA import FMN_Dataset, collate_fn, shape_to_device
from utils.model_eval_functions_PCA import model_id_gen, FMN_id_gen, get_emb_size, def_model, eval_yalm, save_pred_emb_and_loss, eval_network
import time
...
def train_network(cfg):
    ...
    #cache_FMN = osp.join(cache_dir, FMN_id_gen(cfg) +  "_pcaV2")

    # get some important paths
    base_path = osp.dirname(__file__)

    cache_dir_cfg = os.environ.get("DISCO_CACHE_DIR", cfg["dataset"]["cache_dir"])
    cache_dir = cache_dir_cfg if osp.isabs(cache_dir_cfg) else osp.join(base_path, cache_dir_cfg)

    #Creates necessary folders (including the diffusion_net subfolder).
    os.makedirs(cache_dir, exist_ok=True)
    os.makedirs(osp.join(cache_dir, "cache"), exist_ok=True)
    ...
    train_dataset = FMN_Dataset(
        cfg=cfg,
        select_parts=cfg["dataset"]["select_parts"],
        select_tt=cfg["dataset"]["select_tt"],
    ...
    print("Total training time (s):", time.time() - start_total)
    ...
    with torch.no_grad():
        ...
        #Returns train_dataset to be able to pass it to the evaluation.
        return ae_network, train_dataset, log_path
...
if __name__ == "__main__":
    ...
    if osp.isfile(filename):
        ...
        #Force cache_dir from SLURM env if present (so every dataset uses it).
        env_cache = os.environ.get("DISCO_CACHE_DIR", "")
        if env_cache:
            cfg["dataset"]["cache_dir"] = env_cache
        print("CFG dataset.cache_dir =", cfg["dataset"]["cache_dir"])

        torch.manual_seed(cfg["training"]["seed"])
        random.seed(cfg["training"]["seed"])

        #We capture the train_dataset the function returns.
        trained_network, train_dataset, log_path = train_network(cfg)
        start = time.time()
        log_file = open(log_path, "a")
        #Pass trains_dataset to eval_network so that test_dataset can inherit the information of the PCA (U_pca).
        eval_network(trained_network, cfg, train_dataset=train_dataset, calculate_errors=True, calculate_errors_p2s=True,
                     plot_reconstructions=False, plot_embeddings=True, use_cache=True,
                     logfile=log_file)
